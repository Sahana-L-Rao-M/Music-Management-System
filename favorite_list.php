<?php
	...
	$sql = "SELECT * FROM fav_588 where uid='$uid'";
	$result = mysqli_query($conn,$sql);
	...
	while($rows = mysqli_fetch_array($result)){
		...
		if(isset($_POST[$tid])){
			...
			$sql = "DELETE FROM fav_588 WHERE tid='$tid' and uid='$uid'";
			$results = mysqli_query($conn,$sql);

			if($results){
				echo '<script type="text/javascript">';
                echo 'setTimeout(function () { sweetAlert("Removed"," <b>Song '.$track_name.' is removed from your favorite songs</b>","success");';
				echo '}, 500);</script>';
			}else{
				echo '<script type="text/javascript">';
...
<head>
	<title>Symphony Library | Favourites</title>
...
<body>
	...
	<header>
		<div class="container">
			<nav class="navbar navbar-expand-lg navbar-light">
                ...
                <li class="nav-item">
                    ...
                    <p style="color:blue;">| Favorite Songs |</p>
                </li>
      ...
      aria-haspopup="true" aria-expanded="false">Track</button>
								<div class="dropdown-menu dropdown-primary">
									<a class="dropdown-item" href="Songs-tracks.php"><b>All Tracks</b></a>
								</div>	
						</div>
                        <li class="nav-item">
	...
	<!-- //header -->
		...
		<?php
			...
			$sql = "SELECT fav_588.track_name,song_image,audio_file,art_id,alid,fav_588.tid,fav_588.uid  FROM fav_588 INNER JOIN track_588 on fav_588.tid=track_588.tid where fav_588.uid='$uid'";
			$result = mysqli_query($conn,$sql);
			...
			echo "<table width=900>";	
				while($row = mysqli_fetch_array($result)){
					...
					$sql1 = "SELECT * FROM album_588 WHERE alid = '$alid' ";
					$result1 = mysqli_query($conn,$sql1);
					$row1 = mysqli_fetch_array($result1);
					$album_name = $row1['album_name'];

					$sql2 = "SELECT * FROM artist_588 WHERE art_id = '$art_id' ";
					$result2 = mysqli_query($conn,$sql2);
					$row2 = mysqli_fetch_array($result2);
					$artist_name = $row2['artist_name'];


					echo "<form method='post' action='favorite_list.php'>";
					...
					if($count < $limit){
						...
						echo "<td><button type='submit' style='color:red;' class='btn-card' name='$tid'><i class='fa fa-trash-o' style='font-size:15px;'></i></button><br></td><br>";	
					}else{
							$count = 0;
